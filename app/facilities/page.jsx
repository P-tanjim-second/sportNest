...
const ALL_FACILITIES = [
  { id:1,  badge:'Football',   name:'Downtown Turf Arena',    location:'Gulshan, Dhaka',   price:'৳2,500', rating:4.9, reviews:128, capacity:'22 Players', slots:'6 AM–11 PM', grad:'linear-gradient(155deg,#16332A,#234A3B)', accent:'#D4E157' },
  { id:2,  badge:'Badminton',  name:'Birdie Club',             location:'Mirpur, Dhaka',    price:'৳800',   rating:4.8, reviews: 94, capacity:'4 Players',  slots:'5 AM–10 PM', grad:'linear-gradient(155deg,#1A3A4A,#2A5A6B)', accent:'#A8D8EA' },
  { id:3,  badge:'Swimming',   name:'AquaLane Center',         location:'Dhanmondi, Dhaka', price:'৳1,200', rating:4.7, reviews: 76, capacity:'8 Lanes',    slots:'6 AM–9 PM',  grad:'linear-gradient(155deg,#1A4A6B,#2A6B8A)', accent:'#93E4DC' },
  { id:4,  badge:'Tennis',     name:'Riverside Courts',        location:'Banani, Dhaka',    price:'৳1,800', rating:4.9, reviews:112, capacity:'4 Players',  slots:'6 AM–10 PM', grad:'linear-gradient(155deg,#2D4A35,#3D6347)', accent:'#D4E157' },
  { id:5,  badge:'Basketball', name:'Hoop House',              location:'Uttara, Dhaka',    price:'৳1,500', rating:4.6, reviews: 68, capacity:'10 Players', slots:'7 AM–10 PM', grad:'linear-gradient(155deg,#6B2A0E,#C2502E)', accent:'#F4C89A' },
  { id:6,  badge:'Cricket',    name:'Pitch Perfect Arena',     location:'Motijheel, Dhaka', price:'৳3,000', rating:4.8, reviews: 89, capacity:'22 Players', slots:'6 AM–8 PM',  grad:'linear-gradient(155deg,#3A2A1A,#5C4A2A)', accent:'#D4C09A' },
  { id:7,  badge:'Football',   name:'Northside Turf Complex',  location:'Uttara, Dhaka',    price:'৳2,000', rating:4.7, reviews: 55, capacity:'22 Players', slots:'6 AM–11 PM', grad:'linear-gradient(155deg,#16332A,#234A3B)', accent:'#D4E157' },
  { id:8,  badge:'Badminton',  name:'Smash Arena',             location:'Gulshan, Dhaka',   price:'৳900',   rating:4.6, reviews: 42, capacity:'4 Players',  slots:'5 AM–9 PM',  grad:'linear-gradient(155deg,#1A3A4A,#2A5A6B)', accent:'#A8D8EA' },
  { id:9,  badge:'Tennis',     name:'Green Court Tennis',      location:'Dhanmondi, Dhaka', price:'৳1,600', rating:4.8, reviews: 63, capacity:'4 Players',  slots:'6 AM–9 PM',  grad:'linear-gradient(155deg,#2D4A35,#3D6347)', accent:'#D4E157' },
];

const SPORT_TYPES = ['All', 'Football', 'Badminton', 'Swimming', 'Tennis', 'Basketball', 'Cricket'];

function LineArt({ badge }) {
  ...
  return <svg viewBox="0 0 200 160" className="absolute inset-0 h-full w-full opacity-[0.15]" fill="none">{art[badge]}</svg>;
}
...
export default function FacilitiesPage() {
  ...
  const filtered = ALL_FACILITIES.filter(f => {
    const matchType = activeType === 'All' || f.badge === activeType;
      ...
      <div className="mx-auto max-w-7xl px-6 lg:px-10 py-10">
        ...
        {filtered.length > 0 ? (
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {filtered.map(f => (
              <div key={f.id} className="group" style={{ borderRadius:'1.5rem', boxShadow:'var(--shadow-md)', transition:'box-shadow 0.4s, transform 0.4s' }}
                ...
                <div style={{ position:'relative', overflow:'hidden', height:'8rem', background:f.grad, borderRadius:'1.5rem 1.5rem 0 0' }}>
                  <LineArt badge={f.badge} />
                  <div className="absolute inset-0 flex flex-col justify-between p-5">
                    <span style={{ alignSelf:'flex-start', background:'rgba(255,255,255,0.14)', backdropFilter:'blur(8px)', borderRadius:'999px', padding:'0.2rem 0.75rem', fontSize:'10px', fontFamily:'var(--font-mono)', color:f.accent, fontWeight:700, letterSpacing:'0.12em', textTransform:'uppercase' }}>{f.badge}</span>
                    <div>
                ...
                <div style={{ background:'var(--color-surface)', borderRadius:'0 0 1.5rem 1.5rem', padding:'1rem 1.25rem' }}>
                    ...
                    <div className="flex gap-3">
                      {[['Cap',f.capacity],['Hrs',f.slots]].map(([l,v])=>(
                        <div key={l}>
                          ...
                          <p style={{ fontSize:'10px', fontFamily:'var(--font-mono)', color:'var(--color-pine)', marginTop:'2px' }}>{v}</p>
                        </div>
                      ))}
                    </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      <span style={{ fontSize:'1.2rem', fontFamily:'var(--font-display)', color:'var(--color-pine)' }}>{f.price}</span>
                      <span style={{ fontSize:'11px', color:'var(--color-muted)' }}>/hr</span>
                    </div>
                    <a href={`/facilities/${f.id}`} className="group/btn inline-flex items-center gap-1.5 rounded-full px-4 py-2 text-xs font-semibold transition-all duration-300 hover:-translate-y-0.5"
